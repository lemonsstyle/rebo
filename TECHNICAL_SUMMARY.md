...
评论列表与右下资料操作栏使用浅色背景和顶部分隔线区分。评论栏顶部固定显示转发、评论、点赞三种微博风格线性图标：前两个图标按需展开 140 字输入框，转发文本可留空以直接转发；点赞/取消点赞会乐观更新当前详情和来源卡片的计数，失败时回滚并显示原因，发评成功后会刷新评论列表和计数。图标保留可访问名称和悬浮提示。原博主头像位于资料栏左侧，博主名居中，“原文”按钮位于右侧；头像和博主名均可打开该博主主页。右侧评论显示头像、强化后的用户名和评论正文，评论者头像和用户名也链接到各自主页；嵌套层级本身表达回复关系，不重复引用父评论，不显示评论发布时间。评论之间使用淡色分隔线提高辨识度。
