...
- 在详情中显示原文、转发内容、媒体与评论；转发详情可在当前微博和最初原微博的评论间切换，并在不关闭详情时保持背景信息流静止。
...
所有尺寸仍受浏览器视口 14px 边距约束；小屏布局继续铺满可用视口，不强行使用桌面档位。多图详情把缩略图改为主图下方的横向滚动条，利用图片区下方空间，不再占用正文与评论之间的独立列。正文图片查看框和普通媒体预览也随档位等比例收放，避免只改变外框而留下明显空白。纯文字且没有转发内容时切换为上下结构：正文行按实际内容高度增长、最多占详情高度的约 `45%`；评论紧随正文开始，剩余空间由评论列表使用，避免短文字上方留下固定空白。纯文字详情仍按去除空白后的字数选择短、紧凑、中等或默认四档尺寸，并在两列档放大、四列档缩小；长微博保持对应密度的默认大小，避免全文异步加载后频繁改变卡片尺寸。
...
5. 验证正文、详情图片、主图下方缩略图条和评论列表的滚动目标；确认默认详情横图完整填满可用宽度、竖图完整填满可用高度，图片自身不接管滚轮。多图主图左右各 `20%` 区域应分别显示向左、向右箭头并切换相邻图片，中间区域点击后应打开覆盖浏览器可视区域的灯箱并默认完整显示；确认适应窗口时为放大镜光标，点击图片会放大且以点击位置为中心，放大后可按住图片向四个方向拖动。确认底部只有“缩小”“适应窗口”“放大”三个按钮，并分别测试滚轮、三个按钮和键盘缩放；点击图片周围的暗色区域、关闭按钮或按 `Escape` 应关闭并恢复焦点，拖动后的释放动作不应额外缩放或关闭。评论列表滚动到底部应继续加载下一页。
...
10. 打开原创、普通转发及多层转发微博：原创不显示来源切换；转发默认显示当前微博评论，切换“最初原微博”后只加载最深原微博的评论，并同步切换互动按钮和输入草稿。来回切换应保留各自的评论位置与草稿；快速切换或关闭详情后，旧请求不应写入新面板。左侧正文顶部的博主头像/昵称始终展示被点开的微博博主，不随评论来源切换。缺少可用原微博 ID 时不应显示误导性的来源入口。
