...
评论列表与右下资料操作栏使用浅色背景和顶部分隔线区分。转发详情会在评论栏顶部显示“当前微博”和“最初原微博”两个来源标签，默认当前微博；原微博仅在响应中可可靠追溯到带 ID 的最深 `retweeted_status` 时出现，并在第一次切换时请求。切换会同时更新转发、评论、点赞、输入框、评论列表、分页和资料操作栏，避免出现浏览一种微博的评论却向另一条微博互动的歧义；两个来源各自保留已加载评论、分页游标、草稿和滚动位置。前两个图标按需展开 140 字输入框，转发文本可留空以直接转发；点赞/取消点赞会乐观更新当前详情和来源卡片的计数，失败时回滚并显示原因，发评成功后会刷新该来源的评论列表和计数。图标保留可访问名称和悬浮提示。资料栏左侧为当前评论来源的博主头像，博主名居中，“原文”按钮位于右侧；头像和博主名均可打开该博主主页。右侧评论显示头像、强化后的用户名和评论正文，评论者头像和用户名也链接到各自主页；嵌套层级本身表达回复关系，不重复引用父评论，不显示评论发布时间。评论之间使用淡色分隔线提高辨识度。
...
10. 打开原创、普通转发及多层转发微博：原创不显示来源切换；转发默认显示当前微博评论，切换“最初原微博”后只加载最深原微博的评论，并同步切换互动按钮、输入草稿、作者资料和“原文”链接。来回切换应保留各自的评论位置与草稿；快速切换或关闭详情后，旧请求不应写入新面板。缺少可用原微博 ID、主页或永久链接时，不应显示误导性的来源入口或跳往微博首页的“原文”链接。
