(() => {
  ...
  // Thanos 暂不开放图片评论，保留实现便于后续恢复。
  const COMMENT_IMAGES_ENABLED = false;
  ...
  let activeDetailFocusTarget = null;
  ...
  function bridgeRequest(type, payload = {}) {
    return new Promise((resolve) => {
      ...
      }, type === "upload-comment-image" ? 65000 : 8000);
  ...
  function createCardInteractions(status) {
    ...
    const imagePicker = createCommentImagePicker(textarea);
    ...
    const updateComposer = () => {
      ...
      submit.disabled = composerPending
        || imagePicker.__weiboGridImagePicker.isBlocking()
    ...
    composer.addEventListener("submit", async (event) => {
      ...
      if (composerPending || !action || imagePicker.__weiboGridImagePicker.isBlocking()
  ...
  function getCommentReplyCount(comment) {
    if (comment.__weiboGridRepliesLoaded) return getCommentReplies(comment).length;
    const moreInfo = comment?.more_info || comment?.moreInfo || {};
    const text = String(moreInfo.text || moreInfo.highlight_text || "");
    const match = text.match(/(\d[\d,]*)\s*条回复/);
    return Math.max(
      getCommentReplies(comment).length,
      Number(comment.total_number) || 0,
      match ? Number(match[1].replace(/,/g, "")) || 0 : 0
    );
  }
  ...
  function createCommentImagePicker(textarea) {
    const picker = document.createElement("div");
    picker.className = "weibo-grid-reader__comment-image-picker";
    if (!COMMENT_IMAGES_ENABLED) {
      picker.hidden = true;
      picker.style.display = "none";
      picker.__weiboGridImagePicker = {
        getPicId: () => "", isBlocking: () => false,
        clear: () => {}, setDisabled: () => {}
      };
      return picker;
    }
    ...
    const uploadImage = async (file) => {
      if (!file || button.disabled || pending) return;
      if (objectUrl) URL.revokeObjectURL(objectUrl);
      ...
      if (!result.ok || !result.payload?.pid) {
        pid = "";
        status.textContent = `图片上传失败：${result.reason || "请稍后重试"} 请重新选择图片，或移除图片后发送文字。`;
      } else {
    ...
    textarea?.addEventListener("paste", (event) => {
      if (event.defaultPrevented || textarea.disabled || button.disabled) return;
      ...
      if (!file) return; // 普通文字和链接继续交给浏览器粘贴。
      event.preventDefault();
      // 每条评论只支持一张图片；已有图片时替换，上传中不重复提交。
      return uploadImage(file);
    });
    picker.__weiboGridImagePicker = {
      getPicId: () => pid,
      // 已选图片在拿到服务端 pid 前都不能提交，避免静默丢失附件。
      isBlocking: () => pending || Boolean(objectUrl && !pid),
      clear,
  ...
  function createInlineReplyForm(status, comment, comments, detailInteractions, onDone) {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const text = textarea.value.trim();
      const picId = imagePicker.__weiboGridImagePicker.getPicId();
      if (pending || (!text && !picId) || imagePicker.__weiboGridImagePicker.isBlocking()) {
        return;
      }
  ...
  // COMMENT_ACTIONS_REVEAL_DELAY_MS 后按钮一起淡入，自己的评论额外显示删除。
  function attachCommentZones(commentRow, status, comment, comments, detailInteractions) {
    ...
    const currentUserId = detailCommentStates.get(comments)?.currentUserId;
    const authorId = String(comment.user?.idstr || comment.user?.id || "");
    if (currentUserId && authorId === currentUserId && getCommentId(comment)) {
      let deletePending = false;
      const feedback = document.createElement("span");
      feedback.setAttribute("role", "status");
      const deleteZone = createCommentZone("delete", async () => {
        if (deletePending || !window.confirm("确定删除这条评论吗？删除后无法恢复。")) return;
        deletePending = true;
        deleteZone.disabled = true;
        feedback.textContent = "正在删除…";
        rowContent.append(feedback);
        const result = await bridgeRequest("delete-comment", { commentId: getCommentId(comment), authorId });
        if (!result.ok) {
          deletePending = false;
          deleteZone.disabled = false;
          feedback.textContent = `删除失败：${result.reason || "请稍后重试"}`;
          return;
        }
        cleanupCommentTransientPickers(commentRow);
        commentRow.remove();
        status.comments_count = Math.max(0, getNonNegativeCount(status.comments_count) - 1);
        updateStatusMetric(status, "comments", "评论", status.comments_count);
        const state = detailCommentStates.get(comments);
        if (state) {
          // 使正在进行的分页响应失效，重新读取删除后的评论及嵌套回复。
          state.requestVersion += 1;
          state.loading = false;
        }
        await loadDetailComments(status, comments, detailInteractions);
      });
      deleteZone.title = "删除评论";
      deleteZone.setAttribute("aria-label", "删除评论");
      zones.unshift(deleteZone);
    }
  ...
  function createCommentItem(comment, commentsById, renderedCommentIds, postAuthorId, depth = 0, replyController = null) {
    const commentId = getCommentId(comment);
    ...
    const replyLimit = Number.isFinite(comment.__weiboGridReplyLimit)
      ? comment.__weiboGridReplyLimit
      : 2;
    const visibleReplies = replies.slice(0, replyLimit);
    const needsExpand = replyController && replyCount > replyLimit;
    const replyList = document.createElement("div");
    replyList.className = "weibo-grid-reader__comment-replies";
    if (visibleReplies.length) {
      const replyItems = visibleReplies
        .map((reply) => createCommentItem(reply, commentsById, renderedCommentIds, postAuthorId, depth + 1, replyController))
        .filter(Boolean);
    ...
    if (needsExpand) {
      ...
      expand.type = "button";
      expand.className = "weibo-grid-reader__comment-more-replies";
      const nextLimit = replyLimit < 10 ? Math.min(10, replyCount) : replyCount;
      expand.disabled = Boolean(comment.__weiboGridRepliesLoading);
      expand.textContent = comment.__weiboGridRepliesLoading
        ? "加载中…"
        : comment.__weiboGridRepliesError
          ? "加载失败，点击重试"
          : nextLimit >= replyCount
            ? `展开剩余 ${replyCount - replyLimit} 条回复`
            : `展开至 ${nextLimit} 条回复`;
      expand.addEventListener("click", () => {
        if (comment.__weiboGridRepliesError) {
          void preloadCommentReplies(
            replyController.status, replyController.comments,
            replyController.detailInteractions, replyController.state, comment
          );
          return;
        }
        comment.__weiboGridReplyLimit = nextLimit;
        renderDetailComments(
          replyController.status,
          replyController.comments,
          replyController.detailInteractions,
          replyController.state
        );
      });
      replyList.append(expand);
    }
    if (replyList.childNodes.length) {
  ...
  function createActionIcon(type, iconClassName) {
    ...
    if (type === "delete") {
      addPath("M4 6h16M9 6V3h6v3M6 6l1 15h10l1-15M10 10v7M14 10v7");
      return svg;
    }
  ...
  function renderDetailComments(status, comments, detailInteractions, state, append = false) {
    const renderedCommentIds = new Set();
    const existingComments = new Set();
    if (append) {
      comments.querySelectorAll("[data-weibo-grid-comment-state]").forEach((node) => node.remove?.());
      // 分页只追加，避免清空列表触发布局计算后 scrollTop 被压回顶部。
      // 同时保留旧评论里的图片、焦点和未提交的内联回复。
      comments.querySelectorAll(".weibo-grid-reader__comment").forEach((row) => {
        const comment = row.__weiboGridComment;
        if (!comment) return;
        existingComments.add(comment);
        const id = getCommentId(comment);
        if (id) renderedCommentIds.add(id);
      });
    } else {
      cleanupCommentTransientPickers(comments);
      comments.replaceChildren();
    }
    if (state.error) {
      const statePanel = document.createElement("div");
      statePanel.className = "weibo-grid-reader__comment-state weibo-grid-reader__comment-state--error";
      statePanel.dataset.weiboGridCommentState = "error";
      const message = document.createElement("p");
      message.className = "weibo-grid-reader__comment-empty";
      message.setAttribute("role", "alert");
      message.textContent = state.error;
      const retry = document.createElement("button");
      retry.type = "button";
      retry.className = "weibo-grid-reader__comment-retry";
      retry.textContent = "重试加载评论";
      retry.addEventListener("click", () => {
        if (state.loading) return;
        retry.disabled = true;
        retry.textContent = "正在重试…";
        message.textContent = "正在重新加载评论…";
        state.error = "";
        void loadDetailComments(
          status,
          comments,
          detailInteractions,
          Boolean(state.retryAppend)
        );
      });
      statePanel.append(message, retry);
      if (typeof comments.prepend === "function") {
        comments.prepend(statePanel);
      } else {
        comments.append(statePanel);
      }
    }
    ...
    const commentItems = state.comments
      .filter((comment) => !existingComments.has(comment))
      .map((comment) => createCommentItem(
        comment,
        commentsById,
        renderedCommentIds,
        postAuthorId,
  ...
  async function preloadCommentReplies(status, comments, detailInteractions, state, parent) {
    if (parent.__weiboGridRepliesLoading || getCommentReplies(parent).length >= getCommentReplyCount(parent)) return;
    const isCurrent = () => hasValidExtensionContext()
      && state.detailSessionId === activeDetailSessionId
      && comments.isConnected
      && state.comments.includes(parent);
    if (!isCurrent()) return;
    parent.__weiboGridRepliesLoading = true;
    parent.__weiboGridRepliesError = false;
    renderDetailComments(status, comments, detailInteractions, state);
    const replies = [...getCommentReplies(parent)];
    const replyIds = new Set(replies.map(getCommentId).filter(Boolean));
    const cursors = new Set();
    let maxId = "";
    try {
      while (isCurrent()) {
        const result = await bridgeRequest("fetch-comment-replies", {
          statusId: getStatusId(status), parentCommentId: getCommentId(parent), maxId
        });
        if (!isCurrent()) return;
        if (!result.ok) throw new Error(result.reason || "回复加载失败");
        const incoming = Array.isArray(result.payload?.comments) ? result.payload.comments : [];
        const previousCount = replies.length;
        for (const reply of incoming) {
          const replyId = getCommentId(reply);
          if (replyId && !replyIds.has(replyId)) {
        ...
        if (!incoming.length || !nextMaxId || nextMaxId === "0") {
          parent.__weiboGridRepliesLoaded = true;
          break;
        }
        if (cursors.has(nextMaxId) || (maxId && replies.length === previousCount)) {
          throw new Error("回复分页没有继续返回新内容");
        }
        cursors.add(nextMaxId);
        maxId = nextMaxId;
      }
    } catch (error) {
      parent.__weiboGridRepliesError = true;
    } finally {
      parent.__weiboGridRepliesLoading = false;
      if (isCurrent()) renderDetailComments(status, comments, detailInteractions, state);
    }
  }

  function drainCommentReplyPreloads(status, comments, detailInteractions, state) {
    state.replyPreloadQueue ||= [];
    state.replyPreloadQueued ||= new Set();
    state.replyPreloadActive ||= 0;
    while (
      state.replyPreloadActive < MAX_REPLY_PRELOADS
      && state.replyPreloadQueue.length
    ) {
      const parent = state.replyPreloadQueue.shift();
      state.replyPreloadQueued.delete(parent);
      state.replyPreloadActive += 1;
      void preloadCommentReplies(status, comments, detailInteractions, state, parent)
        .finally(() => {
          state.replyPreloadActive -= 1;
          drainCommentReplyPreloads(status, comments, detailInteractions, state);
        });
    }
  }

  function queueCommentReplyPreloads(status, comments, detailInteractions, state, parents) {
    state.replyPreloadQueue ||= [];
    state.replyPreloadQueued ||= new Set();
    state.replyPreloadActive ||= 0;
    for (const parent of parents) {
      if (
        state.replyPreloadQueued.has(parent)
        || parent.__weiboGridRepliesLoading
        || parent.__weiboGridRepliesLoaded
      ) {
        continue;
      }
      state.replyPreloadQueued.add(parent);
      state.replyPreloadQueue.push(parent);
    }
    drainCommentReplyPreloads(status, comments, detailInteractions, state);
  }
  ...
  async function loadDetailComments(status, comments, detailInteractions, append = false) {
    ...
    let result;
    try {
      result = await bridgeRequest("fetch-comments", {
        statusId,
      ...
      result = {
        ok: false,
        reason: error?.message || "评论请求失败，请重试。"
      };
    }
    if (
      !hasValidExtensionContext()
    ...
    if (!result.ok) {
      ...
      if (!append) {
        ...
        renderDetailComments(status, comments, detailInteractions, state, true);
      }
      state.loadMoreSentinel.hidden = true;
      return;
    }
    ...
    const incomingComments = Array.isArray(result.payload?.comments)
      ...
      : [];
    state.currentUserId = String(result.payload?.currentUserId || "");
    ...
    if (!append) {
      ...
      state.replyPreloadQueue?.splice(0);
      state.replyPreloadQueued?.clear();
    }
    ...
    renderDetailComments(status, comments, detailInteractions, state, append);
    if (typeof queueCommentReplyPreloads === "function") {
      queueCommentReplyPreloads(status, comments, detailInteractions, state, incomingComments);
    }
  }

  function closeDetail(restoreHistory = true, restoreFocus = restoreHistory) {
    ...
    const focusTarget = activeDetailFocusTarget || activeDetailAnchor;
    activeDetailSessionId += 1;
    activeDetailStatusId = "";
    activeDetailAnchor = null;
    activeDetailFocusTarget = null;
  ...
  function createDetailCommentContext(status, detailSessionId, contextKey, label) {
    ...
    loading.dataset.weiboGridCommentState = "loading";
    ...
    const state = {
      ...
      loadMoreSentinel,
      replyPreloadQueue: [],
      replyPreloadQueued: new Set(),
      replyPreloadActive: 0
    };
  ...
  function openDetail(status, anchor = null) {
    ...
    activeDetailFocusTarget = document.activeElement instanceof HTMLElement ? document.activeElement : anchor;
