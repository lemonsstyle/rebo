...
3. 点击图文、转发和视频卡片，确认正文中的“查看图片”等链接可直接打开，而不会展开详情卡片。
4. 打开详情后，在正文、图片和评论区分别滚动，确认只有命中的内部区域滚动，背景页面不滚动。
5. 打开单图、多图和“视频+图片”微博，确认媒体可以正常展示与浏览。

### 隐私与限制

扩展只申请 `storage` 权限及微博页面访问权限。为加载评论，它只在微博页面内读取非 HttpOnly 的 `XSRF-TOKEN` 并发送给同源微博接口；不会保存该值，也不会向微博以外的服务上传 Cookie 或信息流内容。
...
3. Open image, repost, and video cards; links such as “查看图片” should open directly instead of expanding the detail card.
...
The extension requests only `storage` permission and access to Weibo pages. To load comments, it reads the non-HttpOnly `XSRF-TOKEN` only inside the Weibo page and sends it only to same-origin Weibo endpoints. It neither stores that value nor uploads cookies or feed content to any external service.
