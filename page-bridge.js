(() => {
  ...
  function crc32Buffer(buffer) {
    // 官方上传的 cs 是文件原始字节的无符号 CRC-32，不是随机数或 MD5 的校验值。
  ...
  async function uploadCommentImage(file) {
    if (!COMMENT_IMAGES_ENABLED) {
      return { ok: false, reason: "当前版本暂未开放图片评论。" };
    }
    ...
    try {
      ...
      endpoint.searchParams.set("cs", String(crc32Buffer(buffer)));
      ...
      const response = await new Promise((resolve, reject) => {
        ...
        // 图床与主站不同源；uid 参数不能代替浏览器的登录 Cookie。
        ...
        request.addEventListener("timeout", () => reject(new Error("图片上传超时，请重新选择图片重试。")));
        request.send(buffer);
      });
      let payload = null;
      try {
  ...
  async function fetchComments(statusId, maxId = "") {
    ...
    try {
      ...
      const resultFlag = payload?.ok ?? payload?.ret;
      if (resultFlag !== undefined && (resultFlag === false || Number(resultFlag) <= 0)) {
        return {
          ok: false,
          reason: payload?.msg || payload?.message || (Number(resultFlag) === -100
            ? "请先登录微博后重试。"
            : "微博未能返回评论，请稍后重试。")
        };
      }
      const comments = [data, data?.data, data?.comments, payload?.comments].find(Array.isArray);
      if (!comments) {
        return { ok: false, reason: "评论响应格式异常，请稍后重试。" };
      }
      ...
      return { ok: false, reason: error instanceof Error ? error.message : "评论请求失败。" };
    }
  }

  async function fetchCommentReplies(statusId, parentCommentId, maxId = "") {
    const endpoint = new URL("/ajax/statuses/buildComments", window.location.origin);
    endpoint.searchParams.set("id", String(parentCommentId));
    endpoint.searchParams.set("is_mix", "0");
    endpoint.searchParams.set("count", "20");
    endpoint.searchParams.set("type", "feed");
    endpoint.searchParams.set("uid", String(getCurrentUserId() || ""));
    endpoint.searchParams.set("fetch_level", "1");
    endpoint.searchParams.set("locale", "zh-CN");
    endpoint.searchParams.set("is_show_bulletin", "2");
    if (maxId) {
      endpoint.searchParams.set("max_id", String(maxId));
    } else {
      endpoint.searchParams.set("is_reload", "1");
    }
    ...
    try {
      ...
      if (!response.ok) {
        return { ok: false, reason: `回复请求失败（HTTP ${response.status}）。` };
      }
      ...
      const replies = [data, data?.data, data?.comments, payload?.comments].find(Array.isArray);
      if (!replies) {
        return { ok: false, reason: "评论回复响应格式异常，请稍后重试。" };
      }
      const explicitMaxId = payload.max_id
        ?? payload.max_id_str
        ?? data?.max_id
      ...
      return {
        ok: false,
        reason: error instanceof Error ? error.message : "评论回复请求失败。"
      };
    }
  }
  ...
  async function createComment(statusId, text, alsoRepost = false, picId = "") {
    if (!COMMENT_IMAGES_ENABLED && picId) {
      return { ok: false, reason: "当前版本暂未开放图片评论。" };
    }
  ...
  async function deleteComment(commentId, authorId) {
    ...
    if (!commentId || !currentUserId || String(authorId || "") !== currentUserId) {
      return { ok: false, reason: "只能删除自己发布的评论。" };
    }
    try {
      // 与官方 iconListDelete 一致，不附带拉黑操作。
      const response = await window.fetch(new URL("/ajax/statuses/destroyComment", window.location.origin), {
        method: "POST",
        credentials: "same-origin",
        headers: { ...createWeiboRequestHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ cid: String(commentId), filter: {} })
      });
      const payload = await response.json();
      if (!response.ok || Number(payload?.ok) !== 1) {
        return { ok: false, reason: payload?.msg || payload?.message || `删除评论失败（HTTP ${response.status}）。` };
      }
      return { ok: true };
    } catch (error) {
      return { ok: false, reason: error instanceof Error ? error.message : "删除评论请求失败。" };
    }
  }

  // 评论点赞/取消点赞使用 object_id（评论 id）、object_type=comment。
  ...
  window.addEventListener("message", (event) => {
    ...
    if (message.type === "fetch-comment-replies") {
      void fetchCommentReplies(message.statusId, message.parentCommentId, message.maxId).then((result) => {
        respond(message.requestId, bridgeSessionId, result);
      });
    }
    ...
    if (message.type === "delete-comment") {
      void deleteComment(message.commentId, message.authorId).then((result) => {
        respond(message.requestId, bridgeSessionId, result);
      });
    }
